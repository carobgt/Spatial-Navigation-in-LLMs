# train_custom.py
# This version is refactored to accept all paths and hyperparameters from the command line.
...
# --- UTILITY FOR CHECKPOINTS (Unchanged) ---
...
def main():
    ...
    # --- Load Components using Parsed Arguments ---
    ...
    # --- Load and Tokenize Data using Parsed Arguments ---
    ...
    # ##################################################################
    # ### CORRECTED METRIC CALCULATION LOGIC STARTS HERE ###
    # ##################################################################

    if training_args.do_eval:
        if "validation" not in tokenized_datasets:
            raise ValueError("--do_eval requires a validation dataset")
        # FIX: Use tokenized_datasets, not lm_datasets which doesn't exist here
    ...
    def compute_metrics(eval_preds):
        ...
        # 1. PER-TOKEN ACCURACY
        # Shift labels and preds for causal LM
        ...
        # 2. SEQUENCE-LEVEL ACCURACY (Cleaner Method)
        num_exact_matches = 0
    ...
    trainer = Trainer(
        ...
        eval_dataset=eval_dataset if training_args.do_eval else None, # Use the potentially truncated eval_dataset
        tokenizer=tokenizer,
